...
const MyPosts = ({ route, navigation }) => {
    const width = useWindowDimensions().width
const {Posts}=route.params

 const delHandle=(id)=>{
     axios.delete(`http://104.131.87.91:2000/post/${id}`).then(res=>navigation.goBack()).catch(err=>console.log(err))

 }
    const deleteHandle=async(id)=>{
        Alert.alert("Warning","Are you sure you want to delet this post?",[{text:"Cancel"},{text:"Continue",onPress:()=>delHandle(id)}])
                        ...
                        <View key={post._id} style={{ marginVertical: 15, marginHorizontal: 5, borderWidth: 1, borderColor: '#ccc' }}>
                            <View style={{ flexDirection: "row", alignItems: 'center', margin: 10 }}>
                                ...
                                <View style={{ marginLeft: 10 }}>
                                    <Text style={{ fontWeight: 'bold', color: 'gray', fontSize: 16 }} >Post created at</Text>

...
import { SafeAreaView,ScrollView, KeyboardAvoidingView, TextInput, Text, TouchableOpacity, StyleSheet, View, useWindowDimensions ,Alert} from 'react-native'
...
const submitHandle=()=>{
    var data={name:name,email:email,phone:phone,address:address,password:password,role:"worker"}
     axios.post('http://104.131.87.91:2000/signup',{...data}).then(res=>{
         if(res.data.success){
AsyncStorage.setItem('token',res.data.data).then(res=> navigation.navigate("Tabs")).catch(err=>console.warn(err))
    ...
    return (
        ...
        defaultValue={phone} style={{ borderColor: 'orange', borderWidth: 2, marginVertical: 10, color: 'black', paddingHorizontal: 20,paddingVertical:15 }} placeholder="PHONE NUMBER" placeholderTextColor="gray"></TextInput>
        ...
        defaultValue={password} style={{ borderColor: 'orange', borderWidth: 2, marginVertical: 10, color: 'black', paddingHorizontal: 20,paddingVertical:15 }} placeholder="PASSWORD" placeholderTextColor="gray"></TextInput>
            ...
            <Text onPress={()=>navigation.navigate("Signin")} style={{ color: "orange", fontSize: 16, fontWeight: 'bold', textAlign: 'center', marginTop: 30 }}>Already have an Account ?</Text>
        </SafeAreaView>
    )
}
